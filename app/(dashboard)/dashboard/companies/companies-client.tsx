...
import { Plus, Building2, Search, BarChart2, Pencil, Trash2, AlertTriangle, Mail } from 'lucide-react'
...
type PageTab = 'companies' | 'invites'

export function CompaniesClient({ initialCompanies, initialInvites, initialMyRequests }: {
  initialCompanies: Company[]
  initialInvites: Invite[]
  initialMyRequests: Invite[]
}) {
  const [tab, setTab] = useState<PageTab>('companies')
  const [companies, setCompanies] = useState<Company[]>(initialCompanies)
  const [invites, setInvites] = useState<Invite[]>(initialInvites)
  const [myRequests] = useState<Invite[]>(initialMyRequests)
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === 'invites' ? (
        <InvitesTab invites={invites} myRequests={myRequests} loading={inviteLoading} onRespond={handleRespond} />
      ) : companies.length === 0 ? (
...
function InvitesTab({ invites, myRequests, loading, onRespond }: {
  invites: Invite[]
  myRequests: Invite[]
  loading: boolean
  onRespond: (inviteId: string, status: 'approved' | 'rejected') => void
}) {
  return (
    <div className="space-y-8">
      <div className="space-y-4">
        <h2 className="font-semibold text-sm">Convites recebidos</h2>
        {invites.length === 0 ? (
          ...
          <div key={inv.id} className="flex items-center justify-between bg-card border border-border rounded-xl p-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => onRespond(inv.id, 'approved')} disabled={loading} className="px-3 py-1 text-xs bg-primary text-primary-foreground rounded-md hover:opacity-90 disabled:opacity-60">Aceitar</button>
              <button onClick={() => onRespond(inv.id, 'rejected')} disabled={loading} className="px-3 py-1 text-xs border border-destructive text-destructive rounded-md hover:bg-destructive/10 disabled:opacity-60">Recusar</button>
            </div>
          </div>
        ))}
      </div>
